...
export default function PokemonList() {
    ...
    const [listOfPokemons, setListPoke] = useState<Pokemon[]>([
      ...
      function onDeleteP(name:string){
        const removepokemon = listOfPokemons.filter(poke => poke.name !== name);
        setListPoke(removepokemon);
      }
        ...
        <div className="grid-pokemon">
            {
                listOfPokemons.map(poke => {
                    return <PokemonBox key={poke.name} {...poke}  onDelete={() => onDeleteP(poke.name)} />
